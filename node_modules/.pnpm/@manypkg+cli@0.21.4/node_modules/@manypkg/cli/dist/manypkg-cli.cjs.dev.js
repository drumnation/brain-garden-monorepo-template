...
var validateNpmPackageName__default = /*#__PURE__*/_interopDefault(validateNpmPackageName);
var parseGithubUrl__default = /*#__PURE__*/_interopDefault(parseGithubUrl);
      ...
      if (deps) {
        for (let depName in deps) {
          ...
          if (!allPackages.has(depName)) {
            if (!semver__namespace.validRange(deps[depName])) {
              continue;
            }
  ...
  for (let [depName, specifierMap] of dependencyRangesMapping) {
    ...
    const maxValue = specifierMapEntryArray.reduce((acc, value) => {
      if (acc[1] === value[1]) {
        ...
        const highestRange = sembear.highest([acc[0], value[0]]);
    ...
    for (let depType of NORMAL_DEPENDENCY_TYPES) {
      ...
      if (deps) {
        for (let depName in deps) {
          ...
          if (mostCommonRange !== undefined && mostCommonRange !== range && semver.validRange(range)) {
            errors.push({
    ...
    for (let depType of NORMAL_DEPENDENCY_TYPES) {
      ...
      if (deps) {
        for (let depName in deps) {
          ...
          if (dependencyWorkspace !== undefined && !range.startsWith("npm:") && !range.startsWith("workspace:") && !semver__namespace["default"].satisfies(dependencyWorkspace.packageJson.version, range)) {
            errors.push({
  ...
  validate: (workspace, allWorkspaces) => {
    ...
    if (peerDeps) {
      for (let depName in peerDeps) {
        ...
        } else if (semver__namespace["default"].validRange(devDeps[depName]) &&
        ...
        semver__namespace["default"].validRange(peerDeps[depName]) && !sembear.upperBoundOfRangeAWithinBoundsOfB(devDeps[depName], peerDeps[depName])) {
          let highestRanges = getMostCommonRangeMap(allWorkspaces);
          let idealDevVersion = highestRanges.get(depName);
...
var INVALID_PACKAGE_NAME = makeCheck({
  ...
  validate: workspace => {
    ...
    let validationErrors = validateNpmPackageName__default["default"](workspace.packageJson.name);
...
var INCORRECT_REPOSITORY_FIELD = makeCheck({
  ...
  validate: (workspace, allWorkspaces, rootWorkspace, options) => {
    var _rootWorkspace$packag;
    let rootRepositoryField = rootWorkspace === null || rootWorkspace === void 0 ? void 0 : (_rootWorkspace$packag = rootWorkspace.packageJson) === null || _rootWorkspace$packag === void 0 ? void 0 : _rootWorkspace$packag.repository;
    if (typeof rootRepositoryField === "string") {
      let result = parseGithubUrl__default["default"](rootRepositoryField);
      if (result !== null && (result.host === "github.com" || result.host === "dev.azure.com")) {
        ...
        if (workspace === rootWorkspace) {
          ...
          if (result.host === "github.com") {
            correctRepositoryField = `${baseRepositoryUrl}/tree/${options.defaultBranch}/${normalizePath__default["default"](workspace.relativeDir)}`;
          } else if (result.host === "dev.azure.com") {
            correctRepositoryField = `${baseRepositoryUrl}?path=${normalizePath__default["default"](workspace.relativeDir)}&version=GB${options.defaultBranch}&_a=contents`;
          }
...
async function upgradeDependency([name, tag = "latest"]) {
  ...
  let {
    ...
    rootPackage,
    rootDir
  } = await getPackages.getPackages(process.cwd());
  let isScope = name.startsWith("@") && !name.includes("/");
  let newVersion = semver__namespace["default"].validRange(tag) ? tag : null;
...
let npmLimit = pLimit__default["default"](40);
...
let execLimit = pLimit__default["default"](4);
...
(async () => {
  ...
  let options = {
    ...defaultOptions,
    ...(rootPackage === null || rootPackage === void 0 ? void 0 : rootPackage.packageJson.manypkg)
  };
