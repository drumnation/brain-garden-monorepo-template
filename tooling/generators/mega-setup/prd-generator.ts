...
export async function generatePRD(config: ProjectConfig): Promise<void> {
  ...
  let prd = template
    ...
    .replace(/\[Project Name\]/g, toTitleCase(config.name))
    .replace(/\[DESCRIPTION\]/g, config.description)
    .replace(/\[Brief description of the product\/feature - 1 sentence\]/g, config.description)
    .replace(/YYYY-MM-DD/g, date);

  // Generate goals based on project type
  const goals = generateGoals(config);
  prd = prd.replace(/1\. \*\*\[Goal 1 Title\]\*\*[\s\S]*?3\. \*\*\[Goal 3 Title\]\*\*[\s\S]*?\n\n/, goals);

  // Generate background context
  const background = generateBackgroundContext(config);
  prd = prd.replace(/\[Paragraph 1: Context and why this is needed\][\s\S]*?\[Paragraph 3: Success criteria or strategic importance\]\n\n/, background);

  // Generate functional requirements
  const functionalReqs = generateFunctionalRequirements(config);
  prd = prd.replace(/\*\*FR-001: \[Requirement Title\]\*\*[\s\S]*?\*\*FR-003: \[Requirement Title\]\*\*[\s\S]*?\n\n/, functionalReqs);

  // Generate non-functional requirements
  const nonFunctionalReqs = generateNonFunctionalRequirements(config);
  prd = prd.replace(/\*\*NFR-001: \[Performance\/Quality Requirement Title\]\*\*[\s\S]*?\*\*NFR-002: \[Performance\/Quality Requirement Title\]\*\*[\s\S]*?\n\n/, nonFunctionalReqs);

  // Generate technical assumptions
  const techAssumptions = generateTechnicalAssumptions(config);
  prd = prd.replace(/\[Description of repository structure\]\n\n[\s\S]*?\[Description of service architecture\]\n\n/, techAssumptions);

  // Generate epic list
  const epicList = generateEpicList(config);
  prd = prd.replace(/1\. \*\*\[Epic 1 Name\]\*\* - \[One-line description\][\s\S]*?4\. \*\*\[Epic 4 Name\]\*\* - \[One-line description\]\n\n/, epicList);
