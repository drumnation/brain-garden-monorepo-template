...
async function updateRootReadme(
  ...
  try {
    ...
    // Try to find and replace existing tables, or append
    if (updated.includes('## Apps') || updated.includes('## Applications')) {
      // Replace apps table
      updated = updated.replace(
        /(## (?:Apps|Applications)[\s\S]*?\|[\s\S]*?\|[\s\S]*?\n)([\s\S]*?)(\n##|\n\n##|$)/,
        `$1${appsTable}\n$3`,
      );
    } else {
      // Append apps section
